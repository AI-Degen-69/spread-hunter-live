...
function renderAnalyticsSurface(kpi, status) {
  ...
  charts.querySelector('#analytics-histogram-card').innerHTML = `<div class="analytics-chart-head"><h3>Inclusive PnL Histogram</h3><small>Every close counts</small></div><div class="analytics-hist-wrap"><div class="analytics-hist">${histogram}</div><div class="analytics-rail"><div class="analytics-rail-track"><span class="analytics-rail-zero" style="left:33%"></span><i class="analytics-rail-band ci-95" style="left:20%;width:58%;background:linear-gradient(90deg,rgba(59,130,246,.20),rgba(59,130,246,.58),rgba(59,130,246,.20))"></i><i class="analytics-rail-band ci-90" style="left:26%;width:46%;background:linear-gradient(90deg,var(--signal),var(--open))"></i><span class="analytics-rail-mean" style="left:48%"></span></div><small>90% CI · negative inclusion: ${lower != null && lower <= 0 ? 'Yes' : 'No'}</small></div></div><div class="analytics-ci-buttons" role="group" aria-label="Confidence interval selection"><button class="active" type="button" data-ci="90" aria-pressed="true">90% Confidence</button><button type="button" data-ci="95" aria-pressed="false">95% Confidence</button></div>`;
